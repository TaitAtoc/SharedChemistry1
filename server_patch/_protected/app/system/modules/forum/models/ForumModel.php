...
class ForumModel extends ForumCoreModel
{
    ...
    public function getSharedChemistryForumHomeTopics($iLimit = 20)
    {
        $iLimit = max(1, (int)$iLimit);

        // SC_FORUM_HOME_TOPICS_CLEAN_PATH_V1_ACTIVE
        $rStmt = Db::getInstance()->prepare(
            'SELECT ' .
            'f.name, ' .
            'f.forumId, ' .
    ...
    public function getReplyPhotos($iMessageId)
    {
        try {
            $rStmt = Db::getInstance()->prepare(
                'SELECT public_path, original_name, mime_type FROM' . Db::prefix('sc_forum_reply_photos') .
                'WHERE message_id = :messageId ORDER BY created_at ASC, photo_id ASC'
            );
