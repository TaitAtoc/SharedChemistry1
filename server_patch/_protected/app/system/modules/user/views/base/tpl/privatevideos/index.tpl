...
.sc-private-media{color:#f7f3ef}.sc-private-media *{box-sizing:border-box}
...
.sc-private-media__hero h1{font-size:32px}.sc-private-media__hero p,.sc-private-media__helper{margin:10px 0 0;color:#b8b3b0;line-height:1.55}
.sc-private-media__grid{display:grid;grid-template-columns:minmax(0,1fr);gap:18px}.sc-private-media__card{padding:24px}
...
.sc-private-media__button{display:inline-flex;align-items:center;justify-content:center;min-height:40px;margin-top:16px;padding:10px 18px;border:0;border-radius:8px;background:linear-gradient(90deg,#ec0868,#c200fb);color:#f7f3ef;font-weight:800;cursor:pointer}
...
.sc-private-media__gallery{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;margin-top:16px}
.sc-private-media__video{display:block;width:100%;aspect-ratio:16/9;border:0;border-radius:8px;background:#101114;object-fit:cover}
...
@media(max-width:800px){main#content{padding:112px 10px 32px!important}.sc-private-media__hero,.sc-private-media__card{padding:20px}.sc-private-media__gallery,.sc-private-access-list{grid-template-columns:1fr}}
</style>
{/literal}
...
<div class="sc-private-media">
    <section class="sc-private-media__hero">
        <h1>{lang 'Private Videos'}</h1>
        <p>{lang 'Upload private videos and manage which trusted couples can view them.'}</p>
    </section>

    {if !empty($private_media_message)}<p class="sc-private-media__notice">{private_media_message}</p>{/if}
    {if !empty($private_media_error)}<p class="sc-private-media__error">{private_media_error}</p>{/if}
        ...
        <section class="sc-private-media__card">
            ...
            {if !empty($privateVideos)}
                <div class="sc-private-media__gallery">
                    {each $video in $privateVideos}
                        {if $video->hasAccess}
                            <video class="sc-private-media__video" src="{% $video->url %}" controls="controls" preload="metadata"></video>
                        {else}
            ...
            {else}
                <p class="sc-private-media__empty">{lang 'No private videos uploaded yet.'}</p>
            {/if}
        </section>

        <section class="sc-private-media__card">
            <h2>{lang 'Access Control'}</h2>
            <p class="sc-private-media__helper">{lang 'Grant or revoke private video access for friends and verified friends.'}</p>
