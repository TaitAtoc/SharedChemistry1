...
class MainController extends Controller
{
    ...
    {
        if ($this->dispatchSharedChemistryPrivateMediaPage()) {
            return;
        }
    ...
    /**
     * pH7's simple router rejects hyphenated controller fragments such as
     * /user/private-photos and falls back to this homepage action. Keep this
     * narrow shim here so the dedicated private media managers still load even
     * when XML route rewrites are stale or bypassed.
     */
    private function dispatchSharedChemistryPrivateMediaPage(): bool
    {
        $sPath = (string)parse_url((string)($_SERVER['REQUEST_URI'] ?? ''), PHP_URL_PATH);
        $sPath = trim($sPath, '/');

        if ($sPath === 'user/private-photos' || $sPath === 'user/private-photos/index') {
            require_once __DIR__ . '/PrivatePhotosController.php';
            (new PrivatePhotosController)->index();
            return true;
        }

        if ($sPath === 'user/private-videos' || $sPath === 'user/private-videos/index') {
            require_once __DIR__ . '/PrivateVideosController.php';
            (new PrivateVideosController)->index();
            return true;
        }
