...
class PictureModel extends PictureCoreModel
{
     ...
     * @return array
     */
    public function getPublicPhotosByProfileId($iProfileId, $iLimit = 4)
    {
        $this->cache->start(self::CACHE_GROUP, 'publicPhotos' . $iProfileId . $iLimit, static::CACHE_TIME);

        if (!$aData = $this->cache->get()) {
            $iLimit = (int)$iLimit;

            $rStmt = Db::getInstance()->prepare(
                'SELECT p.pictureId, p.albumId, p.title, p.file, p.createdDate, a.name, m.username, m.firstName, m.sex FROM ' .
                Db::prefix(DbTableName::PICTURE) . ' AS p INNER JOIN ' .
                Db::prefix(DbTableName::ALBUM_PICTURE) . ' AS a ON p.albumId = a.albumId INNER JOIN ' .
                Db::prefix(DbTableName::MEMBER) . ' AS m ON p.profileId = m.profileId WHERE p.profileId = :profileId ' .
                'AND p.approved = :photoApproved AND a.approved = :albumApproved ORDER BY p.createdDate DESC LIMIT :limit'
            );
