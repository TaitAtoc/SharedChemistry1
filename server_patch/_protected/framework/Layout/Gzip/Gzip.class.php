...
class Gzip
{
    ...
    {
        try {
            // SC_GZIP_LOCAL_FILE_READ_V1_ACTIVE
            $sFullPath = realpath($this->sBase . $sElement);

            if ($sFullPath === false || !$this->isSourceStaticFileExists($sFullPath) || !is_readable($sFullPath)) {
                $this->logStaticFileLoadFailure($sElement);
                return $this->getStaticFileLoadFailureComment($sElement);
            }

            $sFileContents = $this->oFile->getFile($sFullPath);
    ...
    private function getStaticFileLoadFailureComment(string $sElement): string
    {
        return sprintf('/* pH7Builder Gzip: skipped unreadable source file: %s */', $sElement);
    }

    private function logStaticFileLoadFailure(string $sElement): void
    {
        (new Logger())->msg(
            sprintf(self::ERROR_FILE_LOAD_MESSAGE, 'pH7Builder Gzip', $sElement)
        );
    }
