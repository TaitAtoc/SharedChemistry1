...
var Messenger = {
    ...
    // Methods
    hasOpenBoxes: function () {
        for (x in this.aBoxes) {
            if ($("#chatbox_" + this.aBoxes[x]).length > 0 && $("#chatbox_" + this.aBoxes[x]).css('display') != 'none') {
                return true;
            }
        }
    ...
    scheduleHeartbeat: function (iDelay) {
        if (!this.hasOpenBoxes() || this.iHeartbeatFailures >= this.iMaxHeartbeatFailures) {
            this.stopHeartbeat();
            return;
        }
    ...
    createBox: function (sBoxTitle, iMinimizeBox) {
        ...
        this.scheduleHeartbeat(this.iMinHeartbeat);
    },
    heartbeat: function () {
        var iItemsFound = 0;

        if (this.bHeartbeatInFlight || !this.hasOpenBoxes() || this.iHeartbeatFailures >= this.iMaxHeartbeatFailures) {
            this.scheduleHeartbeat(this.iHeartbeatTime);
            return;
        }

        this.bHeartbeatInFlight = true;
        ...
        $.ajax(
            {
                ...
                success: function (oData) {
                    oMe.bHeartbeatInFlight = false;
                    oMe.iHeartbeatFailures = 0;
                    ...
                    oMe.scheduleHeartbeat(oMe.iHeartbeatTime);
                },
                error: function () {
                    oMe.bHeartbeatInFlight = false;
                    ++oMe.iHeartbeatFailures;
                    oMe.iHeartbeatTime = Math.min(oMe.iMaxHeartbeat, oMe.iMinHeartbeat * Math.pow(2, oMe.iHeartbeatFailures));
                    oMe.scheduleHeartbeat(oMe.iHeartbeatTime);
                }
            });
    },
